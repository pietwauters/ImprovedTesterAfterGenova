...
void analyze_compensation_error() {
    ...
    for (int w = 0; w < num_wires; w++) {
        ...
        for (int i = 0; i < num_targets; i++) {
            ...
            printf("%6.1fΩ   %7.1fmV   %9.1fmV    %8.1fmV   %8.1fmV    %8.1fmV    %8.1f%%\n",
                   R_target, V_clean*1000, V_with_wire*1000, exact_error_mv, 
                   simple_estimate*1000, simple_error_mv, simple_error_percent);
        }
        printf("\n");
    }
    ...
    for (int i = 0; i < num_wires; i++) {
        ...
        printf("%5.1fΩ     %8.1fmV     %8.1fmV    %6.1fmV   %6.1f%%\n",
               R_wire, linear_est*1000, exact_val*1000, error_mv, error_percent);
    }
