...
bool Tester::animateSingleWire(int wireIndex) {
    ...
    {
        if((measurements[wireIndex][(wireIndex+1)%3]> 200 ) && (measurements[wireIndex][(wireIndex+2)%3] > 200))
        {
          // OK
          int level = 2;
    ...
    {
        if((measurements[wireIndex][(wireIndex+1)%3]>160) && (measurements[wireIndex][(wireIndex+2)%3]>160))
        {
            // Simply broken
...
State_t Tester::getState() const {
    return currentState;
}

void Tester::setState(State_t newState) {
    currentState = newState;
}

bool Tester::isAllGood() const {
    return allGood;
}

void Tester::startCalibration() {
    DoCalibration = true;
}

void Tester::stopCalibration() {
    DoCalibration = false;
}

void Tester::setReferenceValues(int* refs) {
    myRefs_Ohm = refs;
}

int* Tester::getReferenceValues() const {
    return myRefs_Ohm;
}
